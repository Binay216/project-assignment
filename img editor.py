...
class EdgeEffect(ImageEffect):
    def apply(self, img):
        gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        edges = cv2.Canny(gray, 100, 200)
        return cv2.cvtColor(edges, cv2.COLOR_GRAY2RGB)

class RotateEffect(ImageEffect):
...
class ImageEditorApp:
    ...
    def init_gui(self):
        ...
        self.slider = ttk.Scale(bottom_right, from_=0.1, to=1.0, orient='horizontal', command=self.resize_image)
        self.slider.set(1.0)
    ...
    def create_menu(self):
        ...
        effect_menu.add_command(label="Grayscale", command=lambda: self.apply_effect(GrayscaleEffect()))
        effect_menu.add_command(label="Find Edges", command=lambda: self.apply_effect(EdgeEffect()))
        rotate_menu = tk.Menu(edit_menu, tearoff=0)
        rotate_menu.add_command(label="Rotate 90°", command=lambda: self.apply_effect(RotateEffect(90)))
        rotate_menu.add_command(label="Rotate 180°", command=lambda: self.apply_effect(RotateEffect(180)))
        rotate_menu.add_command(label="Rotate 270°", command=lambda: self.apply_effect(RotateEffect(270)))
        rotate_menu.add_command(label="Rotate 360°", command=lambda: self.apply_effect(RotateEffect(360)))
    ...
    def apply_effect(self, effect: ImageEffect):
        if self.cropped_img is not None:
            self.history.append(self.cropped_img.copy())
            self.cropped_img = effect.apply(self.cropped_img)
            self.resized_for_save = self.cropped_img
            self.redo_stack.clear()
            self.update_cropped_display()
    ...
    def show_user_guide(self):
        guide = (
            ...
            "3. Use the slider to zoom the cropped image.\n"
            "4. Go to File > Save to save the cropped image.\n"
            "5. Use Edit > Effect to apply effects or rotate the image.\n"
        )
